	...
	<body>
						...
						<ul class="mui-table-view">
							...
							<#list viceDirector as item>
								<li data-value="${item.ID}" class="mui-table-view-cell mui-indexed-list-item mui-checkbox mui-left">
									<input type="checkbox" data-tag="user" data-value="${item.ID}"/>${item.realname}
								</li>
							</#list>
							
							<li data-group="部门" class="mui-table-view-divider mui-indexed-list-group">部门</li>
							<#list departs as item>
								<li data-value="${item.ID}" class="mui-table-view-cell mui-indexed-list-item mui-checkbox mui-left">
		...
		<script type="text/javascript" charset="utf-8">
			...
			mui.ready(function() {	
				...
				//var indexedList = new mui.IndexedList(list);
				...
				submitBtn.addEventListener('tap', function() {
					...
					$.each(checkboxArray,function() {
						...
						if (_this[0].checked) {
							var box = $(this);
							if(_this.attr("data-tag")=="user"){
								userValues.push(_this.attr("data-value"));
							}
					...
					if(orgValues.length==0 && userValues.length==0){
						alert("请选择分管领导或部门");
						return false;
					}else{
						$.each(userValues,function(){
							$(list).append('<input type="hidden" name="userId" value="'+this+'">');
						});
					...
					$('form').attr('action','${base}/weixin/petition/saveSelect').submit();
