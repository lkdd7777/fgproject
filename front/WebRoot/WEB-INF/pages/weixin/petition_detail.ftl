...
<body>
		...
		<div class="mui-content">
			<div class="mui-content-padded">
				<#if editable??><#else>
					<#if isReplay??>
						<a href="${base}/weixin/petition/replay?infoId=${info.id}">
							<button class="mui-btn mui-btn-block mui-btn-primary mui-icon">编写信访答复</button>
						</a>
					</#if>
					<#if isRemark??>
						<a href="${base}/weixin/petition/transfer?infoId=${info.id}">
							<button class="mui-btn mui-btn-block mui-btn-primary mui-icon">转签信访件</button>
						</a>
					</#if>
				</#if>
			</div>
